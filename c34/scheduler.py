...
class ExecutionScheduler:
    ...
    def _build_kernel_schedule(self) -> None:
        ...
        for nid in self.graph.node_order:
            ...
            # C3.4 schedules the executable unit consumed by the current CuPy
            # runtime: one high-level graph node.  The C3.2 decomposition is
            # retained as review metadata, but is not misrepresented as an
            # executable kernel sequence until problem #36 is implemented.
            krn = KernelSpecRef(
                kernel_name=f"cupy_node_{node.op_type.lower()}_fp32",
