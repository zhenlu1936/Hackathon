...
## Rotate reports, repeat H200 tests, and clean the framework

Use this procedure when refreshing validation evidence and synchronizing a
working tree with the remote H200 repository:

1. Inventory both worktrees, report directories, Git baselines, and exact file
   hashes before editing. Treat content hashes as authoritative when the two
   repositories have different commit histories.
2. Rename every existing file under `docs/c3_reports/` and
   `docs/c35_reports/` to `old_<name>` exactly once. Update all documentation
   references to those historical paths. Never overwrite or delete an old
   report.
3. Run the remote alltest once as a baseline and retain its complete combined
   stdout/stderr. Ensure the alltest runner continues after rubric diagnostics;
   record required-test failures separately from documented score shortfalls.
4. Run `./run_c35.sh` three times in separate processes with three report paths
   under `/tmp`. Do not place the individual source reports in `docs/`.
5. Aggregate the three source reports with
   `scripts/aggregate_c35_reports.py` into
   `docs/c35_reports/c35_mean_report.json`. Require identical batch sizes,
   model sets, and device names. Preserve each source value and compute every
   reported timing, correctness, plan, stream/event, and memory value with the
   arithmetic mean. Verify `run_count == 3`, all three model runs pass, and
   recompute every mean independently before accepting the report.
6. Update `docs/fix-summary.md` and `docs/remaining-problems.md` with the new
   H200 evidence and any runner defect exposed by the baseline.
7. Audit cleanup candidates before deletion. List tracked deletions separately;
   preserve tracked reports, documentation, evaluator-facing wrappers, and
   ambiguous artifacts. Remove only confirmed caches, bytecode, temporary
   outputs, stale root-level generated reports, and reproducible build output.
8. Run shell syntax checks, compile all submitted Python sources, check for
   empty files and broken symlinks, run `git diff --check`, and validate the
   submission archive. Re-run the six-rule integrity gate over the actual diff
   and executable paths.
9. Synchronize changed files without a blind destructive mirror. Exclude
   `.git/`, `.ssh/`, `.specification/`, bytecode, and caches from content
   manifests; compare SHA-256 manifests before and after synchronization until
   there is no framework-content drift. Synchronize this skill separately when
   `.agents/` is ignored by Git.
10. Run the final remote alltest after the last code sync, replace
    `docs/c3_reports/alltest-report.txt` with that complete transcript, and copy
    the final report back locally. Update both release records again, sync them,
    and verify identical manifests.
11. On the remote repository, inspect `git status`, staged deletions, and the
    staged diff; run `git diff --cached --check`; then commit the complete
    intended tree. Do not push unless the user explicitly requests it. Report
    the commit hash, required-test result, diagnostic shortfalls, and current
    C3.5 means.
