...
_CONV_EPILOGUE_KERNEL: Optional[Any] = None
...
def _execute_fused_conv_epilogue(
    ...
    """Execute direct NCHW convolution, optional residual, and ReLU once."""
    global _CONV_EPILOGUE_KERNEL
    if len(inputs) < 2:
    ...
    weight = xp.ascontiguousarray(xp.asarray(inputs[1], dtype=xp.float32))
    bias_value = inputs[2] if conv_input_count >= 3 else None
    has_bias = int(bias_value is not None)
    bias = (
        xp.ascontiguousarray(xp.asarray(bias_value, dtype=xp.float32))
        if bias_value is not None else x
    )
    residual = (
        xp.ascontiguousarray(
            xp.asarray(inputs[residual_index], dtype=xp.float32)
        ) if has_residual else x
    )
    if x.ndim != 4 or weight.ndim != 4:
        raise ValueError("Fused Conv requires rank-4 NCHW tensors")
    batch, channels, height, width = (int(value) for value in x.shape)
    out_channels, weight_channels, kernel_h, kernel_w = (
        int(value) for value in weight.shape
    )
    kernel_shape = list(attrs.get("kernel_shape", [kernel_h, kernel_w]))
    if len(kernel_shape) != 2 or tuple(map(int, kernel_shape)) != (
        kernel_h, kernel_w
    ):
        raise ValueError("Fused Conv kernel_shape does not match weights")
    ...
    if len(strides) != 2 or len(dilations) != 2:
        raise ValueError("Fused Conv strides/dilations must contain two values")
    stride_h, stride_w = strides
    dilation_h, dilation_w = dilations
    group = int(attrs.get("group", 1))
    if min(kernel_h, kernel_w, stride_h, stride_w,
           dilation_h, dilation_w, group) <= 0:
        raise ValueError("Fused Conv attributes must be positive")
    if (
        channels % group != 0 or out_channels % group != 0
        or weight_channels != channels // group
    ):
        raise ValueError("Fused Conv group is incompatible with channels")
    if has_bias and (bias.ndim != 1 or int(bias.size) != out_channels):
        raise ValueError("Fused Conv bias must match output channels")
    ...
    if auto_pad in {"SAME_UPPER", "SAME_LOWER"}:
        ...
        total_h = max((out_h - 1) * stride_h + effective_h - height, 0)
        total_w = max((out_w - 1) * stride_w + effective_w - width, 0)
        if auto_pad == "SAME_UPPER":
            pad_top, pad_left = total_h // 2, total_w // 2
        else:
            pad_top, pad_left = (total_h + 1) // 2, (total_w + 1) // 2
        pad_bottom, pad_right = total_h - pad_top, total_w - pad_left
    elif auto_pad == "VALID":
        ...
        if len(pads) != 4 or min(pads) < 0:
            raise ValueError("Fused Conv pads must contain four non-negative values")
        pad_top, pad_left, pad_bottom, pad_right = pads
    else:
        raise ValueError(f"Unsupported Fused Conv auto_pad value: {auto_pad}")
    out_h = (height + pad_top + pad_bottom - effective_h) // stride_h + 1
    out_w = (width + pad_left + pad_right - effective_w) // stride_w + 1
    if out_h <= 0 or out_w <= 0:
        raise ValueError("Fused Conv output shape is non-positive")
    output_shape = (batch, out_channels, out_h, out_w)
    if has_residual and tuple(residual.shape) != output_shape:
        raise ValueError(
            f"Fused Conv residual shape {residual.shape} != {output_shape}"
        )
    out = _planned_output_for_shape(attrs, output_shape)

    if _CONV_EPILOGUE_KERNEL is None:
        source = r'''
        extern "C" __global__ void c3_conv_epilogue(
            const float* x, const float* weight, const float* bias,
            const float* residual, float* out,
            long long total, int channels, int height, int width,
            int out_channels, int out_h, int out_w,
            int kernel_h, int kernel_w, int stride_h, int stride_w,
            int dilation_h, int dilation_w, int pad_top, int pad_left,
            int group, int has_bias, int has_residual, int relu) {
          const long long index = (long long)blockIdx.x * blockDim.x + threadIdx.x;
          if (index >= total) return;
          long long value_index = index;
          const int ow = value_index % out_w; value_index /= out_w;
          const int oh = value_index % out_h; value_index /= out_h;
          const int oc = value_index % out_channels; value_index /= out_channels;
          const int batch = (int)value_index;
          const int channels_per_group = channels / group;
          const int outputs_per_group = out_channels / group;
          const int channel_start = (oc / outputs_per_group) * channels_per_group;
          float sum = 0.0F;
          for (int local_c = 0; local_c < channels_per_group; ++local_c) {
            const int ic = channel_start + local_c;
            for (int kh = 0; kh < kernel_h; ++kh) {
              const int ih = oh * stride_h - pad_top + kh * dilation_h;
              if (ih < 0 || ih >= height) continue;
              for (int kw = 0; kw < kernel_w; ++kw) {
                const int iw = ow * stride_w - pad_left + kw * dilation_w;
                if (iw < 0 || iw >= width) continue;
                const long long xi = ((long long)(batch * channels + ic) * height + ih) * width + iw;
                const long long wi = ((long long)(oc * channels_per_group + local_c) * kernel_h + kh) * kernel_w + kw;
                sum = fmaf(x[xi], weight[wi], sum);
              }
            }
          }
          if (has_bias) sum += bias[oc];
          if (has_residual) sum += residual[index];
          if (relu && sum < 0.0F) sum = 0.0F;
          out[index] = sum;
        }
        '''
        _CONV_EPILOGUE_KERNEL = cp.RawKernel(source, "c3_conv_epilogue")
    total = math.prod(output_shape)
    block = 256
    _CONV_EPILOGUE_KERNEL(
        ((total + block - 1) // block,), (block,),
        (
            x, weight, bias, residual, out, cp.int64(total),
            cp.int32(channels), cp.int32(height), cp.int32(width),
            cp.int32(out_channels), cp.int32(out_h), cp.int32(out_w),
            cp.int32(kernel_h), cp.int32(kernel_w), cp.int32(stride_h),
            cp.int32(stride_w), cp.int32(dilation_h), cp.int32(dilation_w),
            cp.int32(pad_top), cp.int32(pad_left), cp.int32(group),
            cp.int32(has_bias), cp.int32(has_residual),
            cp.int32(attrs.get("_activation") == "Relu"),
        ),
    )
    return out
