...
- C3.5 runs the connected optimized graph through CuPy. The working revision
  dispatches individual C3.2 steps through C3.4 bindings and a submitted kernel
  registry. H200 report 8 passes all three golden gates and reports `8.275 s`
  external ResNet wall time with `0.9351` accuracy. Process-accounted memory
  remains open; the direct path reserves a `3.13 GiB` CuPy-pool proxy.

The framework executes inference on the designated H200 AEC device. C3.4's
arena, allocation views, transfers, streams, and events drive the CuPy runtime
in source, and individual C3.2 references now dispatch through that path. The
  primary architectural boundary is making tuning parameters control physical
  launches and obtaining physical launch/overlap evidence.
Additional evaluator ambiguities and scoring limitations are tracked in
...
metadata, attributes, producer/consumer maps, or initializer state. C3.4 plans
and direct C3.2 references now drive the source runtime. Target numerical
qualification of that revised path remains open.
...
| `google.protobuf` (`protobuf`) | `import google.protobuf; google.protobuf.__version__` | H200 probe: `7.35.1` in `docs/c3_reports/old_5.txt` | BSD-3-Clause | `MessageToDict` serialization and `Message` type checking | `c31.import_onnx` (`_json_safe`) |
...
CuPy 14.1.1, ONNX 1.22.0, and protobuf 7.35.1 were confirmed on the remote H200
MIG validation environment in `docs/c3_reports/old_5.txt`. ONNX
...
not vendor them; organizer confirmation of that interpretation remains an open
question.
...
directory, and no precomputed outputs or generated answers. The complete staged
cleanup tree passes this check with 68 members, all 11 required entries, and no
forbidden paths. Ignore rules alone remain policy rather than evidence; rerun
the script whenever the committed submission tree changes.
