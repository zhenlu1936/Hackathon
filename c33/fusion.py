...
def fuse_compute_activation(graph: Graph, fusion_log: List[Dict[str, Any]]) -> int:
    ...
    for node_id in list(graph.node_order):
        ...
        if _is_graph_output(graph, compute_out):
            continue
...
def fuse_execution_regions(graph: Graph,
    ...
    The fused node carries an ordered, self-contained operator program.  This
    utility is intentionally not part of the default pipeline: its reference
    executor runs the retained operations sequentially, so counting the region
    as one launch would violate the C3.3 executable-plan metric.  It remains
    available for explicit region-ABI tests and future code generation.
    """
    if not graph.node_order:
        graph.topological_sort()
